...
def formWholeBodyParser(headers, body): # -> list of form part bodys
    ...
    if len(splits) == 3:
        print("Body.txt has no ending boundary")
        body += boundaryBytes# add ending boundary
    elif len(splits) == 4:
        print("Body.txt has ending boundary")

    assertAns = "Body content splits number: " + str(len(splits)) +  ", please check your form body"        #???????????????????????????????????????????????????????????????????????????????????????
    assert len(splits) == 4 or len(splits) == 3, assertAns
